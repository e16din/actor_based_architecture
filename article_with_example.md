...
Вместо этого я создаю "агентов" - это такие интерфейсы через которые часть приложения(например Fragment) общается с реальными сущностями-акторами(пользователем, сервером, экраном и т.д.).

Грубо говоря каждый фрагмент(активити) это точка входа в программу, и в каждом фрагменте существуют свой набор агентов, которые имплементируют только необходимый функционал для взаимодействия акторов на этом фрагменте.
