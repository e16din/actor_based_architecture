...
import com.e16din.mytaxi.support.KEY_INITIAL_DATA
import com.e16din.mytaxi.support.KEY_RESULT_DATA
...
class SelectRouteFragment : Fragment(), DataKey {
    ...
    inner class SelectRouteScreenAgent {
        ...
        fun saveData(outState: Bundle) {
            outState.putParcelable(dataKey, screenAgent.data)
        }
    ...
    inner class UserAgent {
        ...
        lateinit var onFinishPlaceChanged: (place: Place) -> Unit
    ...
    inner class ServerAgent {
        suspend fun getPlaces(currentPlaceQuery: String): List<Place> {
            val httpClient = (requireActivity().application as MyTaxiApp)
                .httpClient()
